...
public class Polynomial<T extends TypeInterface<T>> {
  ...
  private Pair divAlgorithm(Polynomial<T> left, Polynomial<T> right) {
    ...
    while (remainder.degree() >= right.degree()) {
      ...
      temp.set(remainder.degree() - right.degree(),
              remainder.coefficients.get(remainder.coefficients.size() - 1).div(
  ...
  private void scalarMult(List<T> lst, T scalar) {
    for(int i = 0; i < lst.size(); i++) {
      lst.set(i, lst.get(i).mul(scalar));
    }
  }
  ...
  public static void main(String[] args) {
    ...
    GF num1 = new GF(val1, base);
    GF num2 = new GF(3, base);
    GF num3 = num1.sum(num2); //
    GF num2FromNeg = new GF(-4, base);
    GF num1FromNeg = new GF(-val1, base);
    ...
    System.out.println(num2.sub(num1FromNeg));
    System.out.println(num2.mul(num1FromNeg));
    System.out.println(num2.div(num1FromNeg));
  }
}
